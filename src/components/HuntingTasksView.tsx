...
const HuntingTasksView = forwardRef<HuntingTasksViewRef, {}>((props, ref) => {
  ...
  const renderCard = (week: WeekRecord, isRightCard: boolean = false) => {
    ...
    return (
      <div className="bg-[#FFFFFF] p-6 rounded-[24px] border border-[#E4E4E7] shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col h-full relative">
        {isLoading && (
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="font-semibold text-[17px] text-[#18181B]">{titlePrefix}【狩獵任務】</h3>
          </div>
        ...
        <div className="flex flex-col gap-0.5 flex-1">
                ...
                <div className={`flex items-center py-1.5 px-1 rounded-[14px] transition-colors relative ${(highlightUnfinished && isThisWeek && task.status === 'todo') || (isNextWeek && unwrittenNextWeekTasks.includes(task.id)) ? 'bg-red-50 border border-red-200' : isContentLocked ? '' : 'hover:bg-[#FAFAFA]'}`}>
                  ...
                  {isEditingMain ? (
                    <input
                      ...
                      className="flex-1 bg-transparent text-[14px] font-semibold text-[#18181B] outline-none border-0 border-b border-[#F39C12] focus:ring-0 focus:border-[#F39C12] ml-1 p-0"
                    />
                  ) : (
                    <div 
                  ...
                  {/* Actions Group (Hover) */}
                  {!isContentLocked && (
                    <div className="opacity-0 group-hover:opacity-100 flex items-center absolute right-2 transition-opacity gap-1">
                    <button 
                          ...
                          <Draggable key={sub.id} draggableId={sub.id} index={index}>
                            {(provided, snapshot) => (
                              <div 
                                ...
                                className={`flex items-center py-1 px-1 rounded-[10px] transition-colors group/sub ${snapshot.isDragging ? 'bg-[#FFFFFF] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-[#E4E4E7] z-50' : highlightUnfinished && isThisWeek && sub.status === 'todo' ? 'bg-red-50 border border-red-200' : 'hover:bg-[#FAFAFA]'}`}
                              >
                                <div 
                                  {...provided.dragHandleProps}
                                  className="w-6 h-6 mr-1 opacity-0 group-hover/sub:opacity-100 cursor-grab active:cursor-grabbing text-[#A1A1AA] hover:text-[#F39C12] transition-colors flex items-center justify-center shrink-0"
                                >
                                ...
                                {isEditingSub ? (
                                  <input
                                    ...
                                    onBlur={saveEdit}
                                    onKeyDown={(e) => e.key === 'Enter' && saveEdit()}
                                    className="flex-1 bg-transparent text-[13px] font-medium text-[#18181B] outline-none border-0 border-b border-[#F39C12] focus:ring-0 focus:border-[#F39C12] p-0"
                                  />
                                ) : (
                                  <div 
                                ...
                                {!isContentLocked && (
                                  ...
                                  className="opacity-0 group-hover/sub:opacity-100 w-5 h-5 flex items-center justify-center rounded-full hover:bg-red-50 text-[#A1A1AA] hover:text-red-500 transition-colors absolute right-2"
                                  title="刪除子任務"
                                >
                                  <span className="material-symbols-outlined text-[14px]">remove</span>
                                </button>
          ...
          {!isContentLocked && (
            ...
            >
              <span className="material-symbols-outlined text-[14px]" style={{ fontVariationSettings: "'wght' 200" }}>add</span>
              新增主任務
            </button>
          )}
        </div>
