        ...
        <header className="sticky top-0 z-40 bg-[#FAFAFA]">
          ...
          <div className="h-[48px] px-6 lg:px-10 flex items-center justify-between border-b border-[#E4E4E7]/60">
            <div className="flex items-center gap-1">
              {activeNav === 'hunting_tasks' && ["任務清單", "狩獵任務"].map((tab) => (
                <button 
                  ...
                  className={`px-4 h-8 flex items-center justify-center text-[12px] font-medium tracking-wide rounded-[12px] relative transition-all outline-none focus:outline-none focus:ring-0 focus-visible:outline-none active:outline-none ${activeSubTab === tab ? 'bg-white text-[#18181B] shadow-[0_2px_8px_rgba(0,0,0,0.06)] border border-[#E4E4E7]/40' : 'text-[#71717A] hover:bg-[#F4F4F5] hover:text-[#18181B]'}`}
                >
                  {tab}
                </button>
              ))}
            ...
            <div className="relative" ref={shareRef}>
              <button onClick={() => setIsShareOpen(!isShareOpen)} className={`w-9 h-9 flex items-center justify-center rounded-full transition-colors outline-none focus:outline-none focus:ring-0 focus-visible:outline-none active:outline-none ${isShareOpen ? 'bg-[#F4F4F5] text-[#18181B]' : 'hover:bg-[#F4F4F5] text-[#71717A]'}`} title="分享">
